...
import { getNoShowReports } from "@/lib/server/moderation";
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(await getNoShowReports(fieldId, Number(query.get("page")) || 0, Number(query.get("size")) || 20));
  } catch (error) {
