...
public class PaymentServiceImpl implements PaymentService {
    ...
    public CheckoutResponse createCheckout(UUID userId, CreateCheckoutRequest request) {
        ...
        String returnPath = normalizeReturnPath(request.returnPath());
        ...
        payment.setCheckoutAttempt(payment.getCheckoutAttempt() + 1);
        ProviderCheckoutResult result = strategyFactory.get(provider).createCheckout(
                new ProviderCheckoutRequest(
                        ...
                        payment.getAmount(), currency, booking == null ? null : booking.getUserEmail(), returnPath));
        payment.setProviderSessionId(result.sessionId());
    ...
    public void processWebhook(String payload, String signature) {
        ...
        if (webhookEventRepository.existsById(result.eventId())) {
            log.info("Ignoring duplicate webhook eventId={}", result.eventId());
            return;
        }
        try {
    ...
    private PaymentResponse toResponse(Payment p) {
        return new PaymentResponse(p.getId(), p.getBookingId(), p.getProvider(), p.getPurpose(),
    ...
    private String normalizeReturnPath(String value) {
        ...
        if (!value.startsWith("/") || value.startsWith("//") || value.contains("\\") || value.contains("\r") || value.contains("\n")
                || value.contains("#")) {
            throw new BadRequestException("Invalid checkout return path");
        }
        return value;
    }
}
